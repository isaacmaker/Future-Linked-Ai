...
check = 0
import os
os.chdir(r"C:\Program Files\Future Linked Ai")
...
start_up.iconbitmap("icon_future linked.ico")
img = ImageTk.PhotoImage(Image.open(r"future linked start up .png"))
...
def main():
    ...
    window.iconbitmap("icon_future linked.ico")
    ...
    try:
        ...
        with urllib.request.urlopen("https://raw.githubusercontent.com/isaacmaker/Future-Linked-Ai/main/Ai.json") as url:
            Ai = json.load(url)
            with open("Ai.json","w") as path_file:
                json.dump(Ai,path_file)
                print("Ai saved")
    except:
        try:
            verson = "[not online] " + verson
            with open("Ai.json","r") as path_file:
    ...
    def clear():
        ...
        global answer_label
        global label
        #qs_to_ask = json.load(r"python\questions.json")
        #chats.append("Ai: "+random.randint(0,len(qs_to_ask)-1))
        try:
            label.destroy()
    ...
    def ask(enter1):
        ...
        #Ai = {}
        try:
            enter_sound = vlc.MediaPlayer("enter sound.mp3")
            enter_sound.play()
        except:
            print("no vlc")
        ...
        try:
            #check if question is in Ai
            question = difflib.get_close_matches(question,Ai.keys(),3,0.8)[0]
            question.lower()
            ...
            if question in Ai:
                ...
                none = "none"
                try:
                    Ai_list_none = Ai_ask.get(final_answer)
                except:
                    str(0)
                    Ai_list_none = "none"
                #todo check qustion in Ai_ask before hole list
                if question == "no":
                    Ai_ask = ""
                    clear()
                try:
                    if  Ai_list_none == "":
                        chats.append(Ai_ask.get(final_answer))
                        clear()
                except:
                    str(0)
                    #todo add special qustions
                    #todo add somthing to write papers for a subject
                Ai_ask_1 = Ai.get(question)
                if type(Ai.get(question)) == dict:
                    Ai_ask = Ai.get(question)
                    Ai_list_none = Ai_ask
                    clear()
                try:
                    chats.append("Ai: "+Ai_ask.get(question))
                    if type(Ai.get(question)) == dict:
                        Ai_ask = Ai.get(question)
                        Ai_list_none = Ai_ask
                        clear()
                    ...
                    print("no str")
                if ask_a in Ai_ask:
                    final_answer = chats.append(Ai_ask.get("ask"))
                    try:
                        if final_answer == "no":
                            Ai_ask = ""
                            clear()
                        chats.append(Ai_ask.get(question)) 
                        clear() 
                    except:
                        str(3)  
                else:
                    try:
                        ...
                        chats.append("Ai: "+Ai.get(question))
                        clear()
                        print(" no find")
                try:
                    chatbox.destroy()
            ...
            try:
                ...
                chats.append("Ai: sorry I have no answer for that")
                try:
                    chatbox.destroy()
                    chatbox = tk.Listbox(window,width=58,height=35,background="grey",border=4,)
                    chatbox.grid(row=0,column=0)
